...
import { outdoorListAnac } from "../../utils/constants";

gsap.registerPlugin(ScrollTrigger);

const FacilitiesAnac = () => {
  const btnList = [
    "Swimming",
    "Hiking & Tours",
    "Bird Watching",
  ...
  return (
    ...
    >
      ...
      <div className="border-b-[1px] border-secondaryGreen w-[100px] fac-text"></div>
          ...
          {btnList.map((item, i) => (
            <button
              ...
              className={` px-[10px] pt-[6px] pb-[4px] rounded-lg shadow-lg transform active:scale-75 transition-transform ${
                active == i ? "bg-primaryGreen text-white" : "bg-secondaryGreen"
              }`}
            >
              {item}
            </button>
      ...
      {outdoorListAnac.map((item, i) => (
        ...
        >
          <div className="relative flex flex-col w-[600px] mt-10">
            ...
            />
            <div className="absolute z-20 bg-primaryGreen text-white w-[450px] max-sm:w-[300px] self-center bottom-[-100px] max-sm:bottom-[-10px] lg:left-auto left-[-10px] p-5 rounded-xl fac-subtext">
              ...
              <p className="text-secondaryGreen font-quicksand font-thin text-lg mt-5 max-sm:text-sm fac-subtext">
                {item.desc02}
              </p>
            </div>
          </div>
